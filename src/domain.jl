...
struct GeneralDomainAffect{autonomous,F,T,S,uType} <: AbstractDomainAffect{T,S,uType}
    ...
    function GeneralDomainAffect{autonomous}(
        g::F,
        abstol::T,
        scalefactor::S,
        u::uType,
        resid::uType,
    ) where {autonomous,F,T,S,uType}
...
function isaccepted(
    ...
    t,
    abstol,
    f::GeneralDomainAffect{autonomous,F,T,S,uType},
    resid,
) where {autonomous,F,T,S,uType}
    # calculate residuals
    if autonomous
...
function GeneralDomain(
    ...
    if typeof(u) <: Nothing
        ...
        affect! = GeneralDomainAffect{autonomous}(
            g,
            abstol,
            scalefactor,
            deepcopy(u),
