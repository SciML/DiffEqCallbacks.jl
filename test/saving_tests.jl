using Test, OrdinaryDiffEq, DiffEqProblemLibrary, DiffEqCallbacks
@static if Int==Int64
...
saved_values = SavedValues(Float64, Float64)
saveat = range(prob.tspan..., stop=10, length=50)
...
saved_values = SavedValues(eltype(prob2D.tspan), typeof(prob2D.u0))
saveat = range(prob2D.tspan..., stop=10, length=50)
...
saved_values = SavedValues(Float64, Float64)
saveat = range(prob_inverse.tspan..., stop=10, length=50)
...
saveat = range(prob2D_inverse.tspan..., stop=10, length=50)
