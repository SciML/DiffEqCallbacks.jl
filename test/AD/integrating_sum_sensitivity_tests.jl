...
function adjoint_inplace(du, u, p, t, sol)
    return du .= -vjp((x) -> lotka_volterra(x, p, t), sol(t), u)[1] -
        Zygote.gradient((x) -> g(x, p, t), sol(t))[1]
end
...
sol_nt = solve(prob_nt, Tsit5(), abstol = 1.0e-14, reltol = 1.0e-14)

# total loss functional
function G(p)
    tmp_prob = remake(prob, p = p)
    sol = solve(tmp_prob, Tsit5(), abstol = 1.0e-14, reltol = 1.0e-14)
    res, = quadgk((t) -> g(sol(t), p, t), tspan[1], tspan[2], atol = 1.0e-14, rtol = 1.0e-10)
    return res
end
...
function callback_saving(u, t, integrator, sol)
    temp = sol(t)
    return DiffEqCallbacks.recursive_neg!(
        vjp(
            (x) -> lotka_volterra(temp, x, t), integrator.p, u
        )[1]
    )
end
function callback_saving_inplace(du, u, t, integrator, sol)
    temp = sol(t)
    return du .= -vjp((x) -> lotka_volterra(temp, x, t), integrator.p, u)[1]
end
cb = IntegratingSumCallback(
    (u, t, integrator) -> callback_saving(u, t, integrator, sol),
    integrand_values, deepcopy(p)
)
cb_inplace = IntegratingSumCallback(
    (du, u, t, integrator) -> callback_saving_inplace(
        du,
        u, t, integrator, sol
    ),
...
prob_adjoint = ODEProblem(
    (u, p, t) -> adjoint(u, p, t, sol), [0.0, 0.0],
    (tspan[end], tspan[1]), p; callback = cb
)
prob_adjoint_inplace = ODEProblem(
    (du, u, p, t) -> adjoint_inplace(du, u, p, t, sol),
...
cb_inplace = IntegratingSumCallback(
    (du, u, t, integrator) -> callback_saving_inplace_nt(
        du,
        u, t, integrator, sol
    ),
    integrand_values_inplace_nt, DiffEqCallbacks.allocate_zeros(p_nt)
)
prob_adjoint_nt = ODEProblem(
    (u, p, t) -> adjoint(u, p, t, sol_nt), [0.0, 0.0],
    (tspan[end], tspan[1]), p_nt; callback = cb
)
prob_adjoint_nt_inplace = ODEProblem(
    (du, u, p, t) -> adjoint_inplace(du, u, p, t, sol_nt),
    [0.0, 0.0], (tspan[end], tspan[1]), p_nt; callback = cb_inplace
)

sol_adjoint_nt = solve(prob_adjoint_nt, Tsit5(), abstol = 1.0e-14, reltol = 1.0e-14)
sol_adjoint_nt_inplace = solve(
    prob_adjoint_nt_inplace, Tsit5(), abstol = 1.0e-14,
    reltol = 1.0e-14
)

@test isapprox(dGdp_ForwardDiff, integrand_values.integrand, atol = 1.0e-11, rtol = 1.0e-11)
@test isapprox(
    dGdp_ForwardDiff, integrand_values_inplace.integrand, atol = 1.0e-11, rtol = 1.0e-11
)
@test isapprox(
    dGdp_ForwardDiff,
